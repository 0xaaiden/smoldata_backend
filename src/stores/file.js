    ...
    for (let i = 0; events.length > i; i += 1) {
      ...
      if (config && config.keys) {
        config.keys.forEach((key) => {
          const keyData = JSON.stringify(event.args[key]);
          const indexKey = `${event.event}-${key}-${keyData}`;
          const folderPath = `${this.dbPath}/${event.event}/${key}/${keyData}`;
          const filePath = `${folderPath}/${indexKey}.jsons`;
