...
export default class Ethereum {
  ...
  async getEventsForBlock(startBlock, endBlock, numRetries = 0) {
    ...
    try {
      ...
      if (numRetries < 100) {
        logger.log('warn', `Warning, error happened while contacting RPC, retrying (attempt ${numRetries}), error:\n ${error}`);
        await sleep(11000);
  ...
  async readAllEvents(fromBlock, toBlock, { skipBlocks, batchSize }, fn) {
    ...
    for (const batch of batches) {
      const promises = batch.map(job => this.getEventsForBlock(job.startBlock, job.endBlock));
