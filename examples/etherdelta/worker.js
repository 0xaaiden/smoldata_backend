...
const processMessage = async (message, messageRef, firestoreMessageRef) => {
  ...
  console.log(`endTimestamp is ${endTimestamp} and startTimestamp is ${startTimestamp}`)
  const endBlock = await axios.get(`https://coins.llama.fi/block/ethereum/${endTimestamp}`)
    .then(response => {
  ...
  const startBlock = await axios.get(`https://coins.llama.fi/block/ethereum/${startTimestamp}`)
    .then(response => {
  ...
  // create store
  const IndexingABI = Object.entries(message.events).reduce((acc, [eventName, fields]) => {
  ...
  const indexer = new Indexer(store, abi, address, 'https://eth.llamarpc.com');
  ...
  const output = fs.createWriteStream(path.join(path.resolve(), `/tmp/${messageKey}.zip`));
  archive.pipe(output);
  archive.directory(path.join(path.resolve() + `/tmp/task-index-${messageKey}-db`), false);
  ...
  const url = fileURL[0];
  // get file size
  const fileSize = fs.statSync(path.join(path.resolve(), `/tmp/${messageKey}.zip`)).size;
  ...
  // delete folder
  fs.rmdirSync(path.join(path.resolve() + `/tmp/task-index-${messageKey}-db`), { recursive: true });
...
parentPort.on('message', async ({ message, messageKey }) => {
  ...
  try {
    ...
    console.log(chalk.green('✔'), chalk.bgBlueBright('Message received and ready to be posted'), messageKey);
