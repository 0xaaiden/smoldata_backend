...
import { nodeQueue, getNextNode } from "./nodeQueue";
...
const processNextMessageInQueue = () => {
  if (messageQueue.length > 0 && workerPool.length < MAX_WORKERS) {
    ...
    console.log(
      chalk.bgYellowBright("Processing message from queue"),
      messageKey
    );
    startWorker();
...
const startWorker = () => {
  const workerData = {
    getNextNode,
    // other properties to pass to worker
  };
  ...
  worker.on("message", (result) => {
    console.log(
      chalk.green("✔"),
      chalk.bgYellowBright(`Message returned from worker ${workerId}`),
      result
    );
  ...
  worker.on("error", (error) => {
    console.error(
      chalk.red("✖"),
      chalk.bgRedBright(`Error in worker ${workerId}`),
      error
    );
    workerPool.splice(workerPool.indexOf(worker), 1);
    processNextMessageInQueue();
  ...
  worker.on("exit", (code) => {
    if (code !== 0) {
      console.error(
        chalk.red("✖"),
        chalk.bgRedBright(`Worker ${workerId} stopped with exit code ${code}`)
      );
    }
    workerPool.splice(workerPool.indexOf(worker), 1);
    processNextMessageInQueue();
  ...
  messagesRef.on("child_added", async (snapshot) => {
    ...
    if (message.status === "pending") {
      ...
      if (workerPool.length < MAX_WORKERS) {
        ...
        console.log(
          chalk.bgYellowBright("Worker pool is full, adding message to queue")
        );
